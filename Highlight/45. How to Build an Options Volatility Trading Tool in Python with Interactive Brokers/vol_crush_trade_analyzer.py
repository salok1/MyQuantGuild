...
class IBApp(EWrapper, EClient):
    ...
    def tickPrice(self, reqId, tickType, price, attrib):
        """Receive real-time market data (not used - we use historical data instead)"""
        pass
...
class VolatilityCrushAnalyzer:
    ...
    def setup_market_data_section(self, parent, row):
        """Setup market data input section"""
    ...
    def setup_current_straddle_section(self, parent, row):
        """Setup current straddle pricing section"""
        ...
        ttk.Label(pricing_frame, text="Straddle Price:").grid(row=3, column=0, padx=(0, 10), sticky=tk.W)
        self.straddle_price_label = ttk.Label(pricing_frame, text="$0.00", font=("Arial", 14, "bold"), foreground="blue")
        self.straddle_price_label.grid(row=3, column=1, sticky=tk.W)
        
    def setup_current_greeks_section(self, parent, row):
        """Setup current Greeks section"""
        ...
        self.delta_label = ttk.Label(greeks_frame, text="0.000", font=("Arial", 10, "bold"))
        ...
        self.gamma_label = ttk.Label(greeks_frame, text="0.000", font=("Arial", 10, "bold"))
    ...
    def setup_scenario_section(self, parent, row):
        """Setup scenario analysis section"""
    ...
    def setup_new_greeks_section(self, parent, row):
        ...
        self.new_delta_label = ttk.Label(new_greeks_frame, text="0.000", font=("Arial", 10, "bold"))
        ...
        self.new_gamma_label = ttk.Label(new_greeks_frame, text="0.000", font=("Arial", 10, "bold"))
            ...
            if self.ib_app.connected:
                try:
                    ...
                    if server_version is not None and server_version > 0:
                        ...
                        self.update_status(f"Successfully connected to IB (Server: {server_version})")
                    else:
                        self.update_status("Connected but server version not available")
                except Exception as e:
                    self.update_status(f"Connection established but version check failed: {e}")
            else:
                self.update_status("Failed to connect to Interactive Brokers")
    ...
    def disconnect_ib(self):
        try:
            ...
            self.clear_data()
            self.update_status("Disconnected from Interactive Brokers")
        except Exception as e:
            self.update_status(f"Disconnect error: {e}")
    
    def clear_data(self):
        """Clear all market data and reset displays"""
        self.current_spot = None
        self.current_iv = None
        
        # Reset all labels and clear input fields
        labels_to_reset = [
    ...
    def fetch_market_data(self):
        ...
        # Request historical price data for last closing price
        try:
    ...
    def process_market_data(self):
        ...
        # Get last closing price from historical data
        if 1 in self.ib_app.historical_data and len(self.ib_app.historical_data[1]) > 0:
            price_data = self.ib_app.historical_data[1]
            latest_bar = price_data[-1]  # Most recent bar
            self.current_spot = latest_bar['close']  # Use closing price
            self.update_status(f"Latest closing price: ${self.current_spot:.2f} from {latest_bar['date']}")
        else:
        ...
        if 2 in self.ib_app.historical_data and len(self.ib_app.historical_data[2]) > 0:
            ...
            latest_iv = iv_data[-1]['close']  # Most recent IV
            
            # Convert IV to decimal and annualize if needed
            if latest_iv > 5:
                # Data is in percentage form
                daily_iv_decimal = latest_iv / 100.0
                self.current_iv = daily_iv_decimal * np.sqrt(252)
                self.update_status(f"IV converted from daily % to annualized: {self.current_iv:.1%}")
            else:
                # Data is already in decimal form, annualize
                self.current_iv = latest_iv * np.sqrt(252)
                self.update_status(f"IV annualized from daily decimal: {self.current_iv:.1%}")
        else:
            # Estimate IV (default to 25% annualized)
            self.current_iv = 0.25
            self.update_status("Using estimated IV of 25% (annualized)")
        
        self.update_status(f"Data ready: Spot=${self.current_spot:.2f}, IV={self.current_iv:.1%}")
        
        # Update input fields with fetched data
        self.spot_price_var.set(f"{self.current_spot:.2f}")
        self.strike_var.set(f"{self.current_spot:.2f}")  # Default ATM strike
        self.iv_var.set(f"{self.current_iv*100:.1f}")  # Convert to percentage
        
        # Calculate options with fetched data
        self.price_current_straddle()
        
    def price_current_straddle(self):
        """Price straddle using current input parameters"""
        try:
        ...
        vega = self.calculate_vega(spot_price, strike_price, T, r, iv_decimal) * 2  # Call + Put vega
        ...
        # Set default scenario values
        if not self.new_spot_var.get():
            self.new_spot_var.set(f"{spot_price:.2f}")
        if not self.new_iv_var.get():
            self.new_iv_var.set(f"{iv_percent:.1f}")
            
        self.update_status(f"Straddle priced: ${straddle_price:.2f} (Call: ${call_price:.2f}, Put: ${put_price:.2f})")
        
    def analyze_scenario(self):
        """Analyze scenario with new spot and IV"""
        try:
            new_spot = float(self.new_spot_var.get())
            new_iv = float(self.new_iv_var.get()) / 100.0  # Convert from percentage
        except ValueError:
            messagebox.showerror("Error", "Invalid spot price or IV values")
            return
            
        # Get current strike price from input field
        try:
            K = float(self.strike_var.get())  # Use current strike from input
            days_to_expiry = int(self.days_var.get())
        except ValueError:
        ...
        self.update_status(f"Scenario complete: New price ${new_straddle_price:.2f}, Long P/L ${pnl_long:+.2f}")
